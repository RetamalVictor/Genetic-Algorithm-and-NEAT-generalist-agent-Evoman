...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self, dt, game):
        ...
        if game.enemymode == "static":  # Enemy controlled by static movements.
            ...
            if (
                game.player.rect.right < game.enemy.rect.left
                and abs(game.player.rect.right - game.enemy.rect.left) <= 50
            ) or (
                game.enemy.rect.right < game.player.rect.left
                and abs(game.enemy.rect.right - game.player.rect.left) <= 50
            ):
                atack3 = 1
            else:
                atack3 = 0
        ...
        if game.start == 1:
            ...
            for cell in game.tilemap.layers["triggers"].collide(
                ...
                if "t" in blockers:
                    ...
                    game.player.rect.x += (
                        self.direction_floor * 100 * dt
                    )  # moves player over the moving floor
            ...
            if (self.resting == 1 and atack1 == 1) or (
                self.resting == 1 and game.player.atacked == 1
            ):
                self.dy = -900
                self.resting = 0
            ...
            if atack2 == 1 and not self.gun_cooldown:
                ...
                for i in range(0, aux):
                    self.twists.append(
                        Bullet_e5(
                            (
                                self.rect.x + (self.direction * (i * 30)),
                                self.rect.top + (self.direction * (i * 20)),
                            ),
                            self.direction,
                            game.player.rect,
                            len(self.twists),
...
class Bullet_e5(pygame.sprite.Sprite):
    ...
    def update(self, dt, game):
        ...
        if (
            ...
            or self.rect.bottom < 1
            or self.rect.top > 512
        ):
            self.kill()
